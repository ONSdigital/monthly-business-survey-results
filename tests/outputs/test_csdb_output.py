...
def input_df(filepath):
    return pd.read_csv(filepath / "input_df_2.csv", index_col=False)


@pytest.fixture(scope="class")
def output_df(filepath):
    return pd.read_csv(filepath / "output_df_2.csv", index_col=False)
...
class TestCSDBOutput:
    def test_csdb_output(
        ...
        actual_output = create_csdb_output(
            ...
            cdid_data_path=Path(filepath / "cdid_mapping_2.csv"),
        )
