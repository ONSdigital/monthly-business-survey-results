...
class TestFilters:
    def test_basic_filter(self, scenario, filters, imputation_data_dir):
        """Test ignore_from_link is correct"""

        df_output_expected = pd.read_csv(
            imputation_data_dir / "link_filter" / (scenario + ".csv")
        )

        df_filters = pd.read_csv(
            imputation_data_dir / "link_filter" / (filters + ".csv")
        )
    ...
    def test_exception(self, scenario, filters, imputation_data_dir):
