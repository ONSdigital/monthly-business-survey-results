...
def aggregate_weights_by_domain_test_input(filepath):
    return pd.read_csv(filepath / "aggregate_weights_by_domain_input.csv", 
                       index_col=False)


@pytest.fixture(scope="class")
def aggregate_weights_by_domain_test_output(filepath):
    return pd.read_csv(filepath / "aggregate_weights_by_domain_output.csv", 
                       index_col=False)


class TestAggregateWeightsByDomain:
    def test_aggregate_weights_by_domain(self, 
