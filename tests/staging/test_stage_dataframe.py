...
def test_drop_derived_questions(filepath):
    df_input = pd.read_csv(
        filepath / "drop_derived_questions_input.csv", dtype={"form_type": "str"}
    )
    ...
    expected_output = pd.read_csv(
        filepath / "drop_derived_questions_output.csv", dtype={"form_type": "str"}
    )
...
def test_drop_derived_questions_multiple(filepath):
    df_input = pd.read_csv(
        filepath / "drop_derived_questions_multi_input.csv", dtype={"form_type": "str"}
    )
    ...
    expected_output = pd.read_csv(
        filepath / "drop_derived_questions_multi_output.csv", dtype={"form_type": "str"}
    )
