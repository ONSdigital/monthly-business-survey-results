...
format. 
    ...
    nas_or_derived_mask = (df[config["imputation_marker_col"]].isna()) | (df[config["imputation_marker_col"]]=="d")
    
    df = df[~nas_or_derived_mask]

    backdata_contributors = df[[config['reference'], config['period'], config['status'], config['nil_status_col']]]
    
    # Hard coding response variable name, is not needed but keeping it
    # to match current back data format
    
    backdata_responses = df[[config['reference'], config['period'], config['question_no'], 'response', config['target'], config['imputation_marker_col']]]
    
    backdata_responses = backdata_responses[backdata_responses["period"]==backdata_period]
    
    backdata_contributors = backdata_contributors[backdata_contributors[config["period"]] == backdata_period]
    ...
    backdata_responses = backdata_responses.rename(columns={config["imputation_marker_col"]:"imputationmarker"})
