...
_active_config: Optional[Dict] = None  # Store active config for use in handlers
...
def _handle_uncaught_exception(exc_type, exc_value, exc_tb):
    """Handle uncaught exceptions, ensure logs are flushed."""
    ...
    try:
        ...
        # If logging fails, ensure original traceback still appears
        sys.__excepthook__(exc_type, exc_value, exc_tb)
    finally:
...
def configure_logger_with_run_id(config: Dict) -> logging.Logger:
    ...
    run_id = str(_active_config["run_id"])
