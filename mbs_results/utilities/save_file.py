...
def save_file(
    ...
    if config["platform"] == "s3":
        ...
        raise ValueError(
            f"Unknown platform [{config['platform']}]. Only 's3' (S3) or 'network' are supported."
        )
...
def save_dataframe_locally(
    ...
    try:
        ...
        if file_format == "csv":
            ...
            raise ValueError(
                f"Invalid file format [{file_format}]. Supported formats are 'csv', 'json', or 'parquet'."
            )

        logger.info(
            f"DataFrame successfully saved locally at '{file_path}' as {file_format} format."
        )
