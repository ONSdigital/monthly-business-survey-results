...
def is_object_present(s3_client: boto3.client, bucket_name: str, s3_object_key: str) -> bool:
    ...
    try:
        ...
        raise Exception(f"Failed to check if the object exists in bucket {bucket_name}. Error: {str(e)}")
...
def create_folder_if_not_exists(s3_client: boto3.client, bucket_name: str, folder_name: str) -> None:
    """
    ...
    try:
        ...
        # Catch all other exceptions and raise a generic exception
        logger.error(f"An unexpected error occurred: {e}")
        raise Exception(f"Failed to create folder {folder_name} in bucket {bucket_name}. Error: {str(e)}")
...
def upload_dataframe_to_s3(
    ...
    compression: Union[str, None] = None  # Compression type for CSV/Parquet (e.g., 'gzip', 'snappy', or None)
) -> None:
    """
...
def load_dataframe_from_s3(
    ...
    try:
        ...
        logger.error(f"An unexpected error occurred while reading DataFrame from S3: {e}")
        raise Exception(f"Failed to read DataFrame from S3 at {s3_key}. Error: {str(e)}")